...
// TestDeleteEntryByFileName verifies deleting an entry by FileName.
func TestDeleteEntryByFileName(t *testing.T) {
    name := "deleteByFileNameTest"
    rootPath := "/content/"
    delete(indices, name) // clean slate

    idx := NewIndex(name, rootPath)

    t.Run("non-existent FileName returns error", func(t *testing.T) {
        err := idx.DeleteEntryByFileName("unknown.md")
        if err == nil {
            t.Errorf("expected error for deleting non-existent FileName, got nil")
        }
    })

    t.Run("valid FileName deletes successfully", func(t *testing.T) {
        entry := IndexEntry{
            Id:       "entry-456",
            FileName: "my-other-file.md",
            Title:    "Another Title",
            Path:     "another.html",
        }
        idx.AddEntry(entry)
        ...
        err := idx.DeleteEntryByFileName("my-other-file.md")
        if err != nil {
            t.Fatalf("unexpected error deleting by FileName: %v", err)
        }
        ...
        if idx.GetEntry("entry-456") != nil {
            t.Errorf("expected nil when retrieving deleted entry by ID, got non-nil")
        }
    })
}
